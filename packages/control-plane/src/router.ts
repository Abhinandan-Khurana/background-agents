...
import { listBitbucketRepos } from "./auth/bitbucket";
...
async function handleListRepos(
  ...
  if (cached) {
    ...
    if (!isFresh && ctx.executionCtx && vcsProvider === "github") {
      // Stale — serve immediately but refresh in background (GitHub only)
