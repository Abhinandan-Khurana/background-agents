...
class VCSCredentials(NamedTuple):
    ...
    def redact_from(self, text: str) -> str:
        ...
        if self.token:
            ...
            if encoded_token != self.token and encoded_token in result:
                result = result.replace(encoded_token, "[REDACTED]")

        if self.username and self.username not in ("x-access-token", "x-token-auth"):
            encoded_username = urllib.parse.quote(self.username, safe='')
            if encoded_username != self.username and encoded_username in result:
...
class AgentBridge:
    ...
    async def _handle_push(self, cmd: dict[str, Any]) -> None:
        ...
        embedding them in URLs (for GitHub), preventing exposure via process listing.
        For Bitbucket, embeds URL-encoded credentials because GIT_ASKPASS is unreliable.
        """
        ...
        try:
            ...
            # For Bitbucket, use embedded credentials (URL encoded) because GIT_ASKPASS is unreliable
            if vcs_provider == "bitbucket":
                encoded_username = urllib.parse.quote(vcs_creds.username, safe='')
                encoded_token = urllib.parse.quote(vcs_creds.token, safe='')
                remote_url = f"https://{encoded_username}:{encoded_token}@bitbucket.org/{repo_owner}/{repo_name}.git"
            else:
                # For GitHub, use public URL and rely on GIT_ASKPASS
                remote_url = f"https://github.com/{repo_owner}/{repo_name}.git"

            # Create a temporary askpass script to provide credentials securely for GitHub
            askpass_script = None
            git_env = os.environ.copy()

            # Setup GIT_ASKPASS for GitHub
            if vcs_provider != "bitbucket":
                try:
