...
class SandboxSupervisor:
    ...
    def _get_clone_url(self) -> str:
        """Get clone URL with embedded credentials for Bitbucket, public URL for GitHub.

        Bitbucket requires credentials embedded in the URL because GIT_ASKPASS
        is not reliably invoked by git for Bitbucket HTTPS authentication.
        GitHub uses GIT_ASKPASS instead (set up in perform_git_sync).
        """
...
""")
        ...
        try:
            ...
            if not self.repo_path.exists():
                ...
                # For Bitbucket, reset remote URL to public version to avoid
            ...
            # For Bitbucket, temporarily set authenticated remote URL since GIT_ASKPASS
            # is unreliable; for GitHub, GIT_ASKPASS handles credentials
    ...
    async def _quick_git_fetch(self) -> None:
        ...
        For Bitbucket, uses authenticated URL directly; for GitHub, uses GIT_ASKPASS.
        """
        if not self.repo_path.exists():
