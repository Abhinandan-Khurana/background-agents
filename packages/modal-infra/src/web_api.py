...
@app.function(
    ...
    secrets=[github_app_secrets, bitbucket_secrets, internal_api_secret],
)
...
async def api_create_sandbox(
    ...
    try:
        ...
        session_config = SessionConfig(
            ...
            provider=str(request.get("provider", "anthropic")),
            model=str(request.get("model", "claude-sonnet-4-5")),
            git_user=git_user,
            vcs_provider=str(vcs_provider),
        )
        ...
        config = SandboxConfig(
            ...
            control_plane_url=str(control_plane_url or ""),
            sandbox_auth_token=str(request.get("sandbox_auth_token") or ""),
...
@app.function(
    image=function_image,
    volumes={"/data": inspect_volume},
    secrets=[github_app_secrets, internal_api_secret, bitbucket_secrets],
)
@fastapi_endpoint(method="POST")
...
@app.function(image=function_image, secrets=[github_app_secrets, bitbucket_secrets, internal_api_secret])
@fastapi_endpoint(method="POST")
